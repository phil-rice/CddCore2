...
import org.cddcore.engine.enginecomponents.{EngineComponent, CannotAddScenarioException, Scenario}
...
class ConflictingScenariosSpec extends CddNonRecursiveSpec[String, String] {
  ...
  implicit val lifeCycle = new NullLifeCycle[EngineComponent[String, String]]
  ...
  "Two conflicting scenarios without whys" should "report an error" in {
    ...
    intercept[CannotAddScenarioException[_, _]](DecisionTree(mockEngine, Seq(scenario1, scenario2))).getMessage shouldBe
