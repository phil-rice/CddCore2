...
class EngineSpec extends CddSpec {
  ...
  "An engine " should "have use cases added " in {
    ...
    uc1 shouldBe UseCase("some usecase1", List(), None, "(EngineSpec.scala:22)")
    uc2 shouldBe UseCase("some usecase2", List(), Some("comment"), "(EngineSpec.scala:23)")
    uc3 shouldBe UseCase("usecase3", List(), None, "(EngineSpec.scala:24)")
  }
