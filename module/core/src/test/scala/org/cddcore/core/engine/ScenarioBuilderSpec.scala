...
class ScenarioBuilderSpec extends CddSpec {
  ...
  "A <situation> produces <result>" should "make a SituationAndResultScenario" in {
    ...
    s shouldBe Scenario[Int, String](situation = 1, assertion = EqualsAssertion("result"), reason = SimpleReason("result"), definedInSourceCodeAt = "(ScenarioBuilderSpec.scala:9)")
  ...
  it should "throw BecauseNotTrueException when the because is not true for the situation" in {
    ...
    }.getMessage shouldBe "Scenario defined at (ScenarioBuilderSpec.scala:44) cannot be added because the reason given isn't actually true"
  }
  it should "have allScenarios just including itself" in {
  ...
  it should "throw BecauseNotTrueException when the when is not true for the situation" in {
    ...
    }.getMessage shouldBe "Scenario defined at (ScenarioBuilderSpec.scala:74) cannot be added because the reason given isn't actually true"
  }
